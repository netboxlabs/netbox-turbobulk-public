...
class TestClientInitialization(unittest.TestCase):
    ...
    def test_init_from_env_vars(self):
        ...
        with patch.dict('os.environ', {'NETBOX_URL': 'http://env-netbox:8080', 'NETBOX_TOKEN': 'env-token'}):
...
class TestGetTemplate(unittest.TestCase):
    ...
    def test_get_template_returns_required_fields_only(self):
        ...
        mock_schema = {
            'fields': [
                {'name': 'id', 'type': 'AutoField', 'primary_key': True, 'nullable': False, 'default': None},
                {'name': 'name', 'type': 'CharField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'slug', 'type': 'SlugField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'status', 'type': 'CharField', 'primary_key': False, 'nullable': False, 'default': 'active'},
                {'name': 'description', 'type': 'TextField', 'primary_key': False, 'nullable': True, 'default': ''},
            ]
        }
    ...
    def test_get_template_fk_uses_id_suffix(self):
        ...
        mock_schema = {
            'fields': [
                {'name': 'id', 'type': 'AutoField', 'primary_key': True, 'nullable': False, 'default': None},
                {'name': 'name', 'type': 'CharField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'site', 'type': 'ForeignKey', 'primary_key': False, 'nullable': False, 'default': None, 'foreign_key': 'dcim.site'},
            ]
        }
    ...
    def test_get_template_generates_correct_defaults_by_type(self):
        ...
        mock_schema = {
            'fields': [
                {'name': 'name', 'type': 'CharField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'count', 'type': 'IntegerField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'active', 'type': 'BooleanField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'data', 'type': 'JSONField', 'primary_key': False, 'nullable': False, 'default': None},
                {'name': 'rate', 'type': 'DecimalField', 'primary_key': False, 'nullable': False, 'default': None},
            ]
        }
    ...
    def test_load_sends_dry_run_parameter_in_form_data(self):
        ...
        try:
            ...
            with patch.object(self.client.session, 'post', return_value=mock_response) as mock_post:
                with patch.object(self.client, '_wait_for_job', return_value={'valid': True, 'rows': 1}):
    ...
    def test_load_dry_run_operation_label(self):
        ...
        try:
            ...
            with patch.object(self.client.session, 'post', return_value=mock_response):
                with patch.object(self.client, '_wait_for_job', return_value={'valid': True}) as mock_wait:
                    self.client.load('dcim.site', parquet_path, dry_run=True)
...
class TestDeleteDryRun(unittest.TestCase):
    ...
    def test_delete_sends_dry_run_parameter_in_form_data(self):
        ...
        try:
            ...
            with patch.object(self.client.session, 'post', return_value=mock_response) as mock_post:
                with patch.object(self.client, '_wait_for_job', return_value={'valid': True, 'rows': 3}):
